import PaymentModal from "./JS/payment.js";
...
async function hideOrderForUser(orderId) {
  ...
  await updateDoc(doc(db, "clients", user.uid, "orders", orderId), {
    userHiddenByClient: true,
    userHiddenAt: new Date().toISOString(),
  });
}
...
async function hideOperationForUser(orderId, kind) {
  if (kind === "withdrawal") {
    const user = auth.currentUser;
    if (!user || !orderId) return;
    await updateDoc(doc(db, "clients", user.uid, "withdrawals", orderId), {
      userHiddenByClient: true,
      userHiddenAt: new Date().toISOString(),
    });
    return;
...
export function mountSoldeModal(options = {}) {
  ...
  window.addEventListener("withdrawalSubmitted", () => {
    attachWithdrawalsListener();
  });
